...
def _load_real_race_state(file_mtime_ns: int) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    with HISTORY_PATH.open("r", encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle)
        for row in reader:
            if (row.get("race_date") or "").strip() == latest_date:
    ...
    for race_id, rows in sorted(grouped_rows.items()):
        ...
        result_order = [int(row.get("gate") or 0) for row in finished_runners if int(row.get("gate") or 0) > 0]
