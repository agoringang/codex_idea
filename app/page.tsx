...
const meetingPrograms: MeetingProgram[] = [
  { key: "tokyo", date: "2026-05-02", day: "土曜", venue: "東京", meeting: "2回東京3日", times: ["10:05", "10:40", "11:10", "11:40", "12:30", "13:00", "13:30", "14:00", "14:30", "15:05", "15:45", "16:30"] },
  { key: "kyoto", date: "2026-05-02", day: "土曜", venue: "京都", meeting: "3回京都3日", times: ["10:00", "10:30", "11:00", "11:30", "12:20", "12:50", "13:20", "13:50", "14:20", "14:55", "15:30", "16:10"] },
  { key: "niigata", date: "2026-05-02", day: "土曜", venue: "新潟", meeting: "1回新潟1日", times: ["09:45", "10:20", "10:50", "11:20", "12:10", "12:40", "13:10", "13:40", "14:15", "14:50", "15:20", "16:00"] },
  { key: "tokyo", date: "2026-05-03", day: "日曜", venue: "東京", meeting: "2回東京4日", times: ["10:00", "10:30", "11:00", "11:30", "12:20", "12:50", "13:20", "13:50", "14:25", "15:00", "15:25", "16:10"] },
  { key: "kyoto", date: "2026-05-03", day: "日曜", venue: "京都", meeting: "3回京都4日", times: ["10:00", "10:30", "11:00", "11:30", "12:20", "12:50", "13:20", "13:50", "14:25", "15:00", "15:40", "16:25"] },
  { key: "niigata", date: "2026-05-03", day: "日曜", venue: "新潟", meeting: "1回新潟2日", times: ["09:50", "10:20", "10:50", "11:20", "12:10", "12:40", "13:10", "13:40", "14:15", "14:50", "15:15", "16:00"] },
];

const raceOverrides: Record<string, RaceOverride> = {
  "tokyo-20260502-10": { title: "スイートピーステークス", grade: "L", course: "芝1800m 左", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "tokyo-20260502-11": {
    title: "京王杯スプリングカップ",
    grade: "GII",
    course: "芝1400m 左",
  ...
  "kyoto-20260502-10": { title: "京極特別", grade: "2勝クラス", course: "ダート1200m 右", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "kyoto-20260502-11": {
    title: "ユニコーンステークス",
    grade: "GIII",
    course: "ダート1900m 右",
  ...
  "kyoto-20260502-12": { title: "4歳以上1勝クラス", grade: "1勝クラス", course: "芝1600m 右外", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "niigata-20260502-1": { title: "障害4歳以上未勝利", grade: "未勝利", course: "障害芝2890m", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "niigata-20260502-11": { title: "三条ステークス", grade: "3勝クラス", course: "ダート1800m", officialNote: "JRA公式の今週の開催予定。詳細出馬表の接続待ち。" },
  "tokyo-20260503-11": { title: "プリンシパルステークス", grade: "L", course: "芝2000m 左", officialNote: "JRA公式出馬表を確認中。日本ダービーにつながる3歳戦。" },
  "kyoto-20260503-3": { title: "3歳未勝利", grade: "未勝利", course: "ダート1800m 右", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "kyoto-20260503-6": { title: "3歳1勝クラス", grade: "1勝クラス", course: "ダート1400m 右", officialNote: "JRA公式出馬表を確認。詳細データ接続待ち。" },
  "kyoto-20260503-11": {
    title: "天皇賞（春）",
    grade: "GI",
    course: "芝3200m 右外",
  ...
  "niigata-20260503-11": { title: "越後ステークス", grade: "OP", course: "ダート1200m", officialNote: "JRA公式の今週の開催予定。詳細出馬表の接続待ち。" },
};

function defaultTitle(raceNumber: number) {
  if (raceNumber <= 4) return "未勝利・条件戦";
  if (raceNumber <= 8) return "平場戦";
  if (raceNumber <= 10) return "特別戦";
  if (raceNumber === 11) return "メインレース";
  return "最終レース";
}
...
const liveSnapshots: Record<string, LiveSnapshot> = {
  "kyoto-20260503-11": {
    ...
    updatedAt: "13:45",
    oddsMoves: [
      { number: 12, name: "ヘデントール", previousOdds: 3.5, currentOdds: 3.2, direction: "down", reason: "本命側に買いが入っています" },
      { number: 14, name: "ホーエリート", previousOdds: 15.1, currentOdds: 12.8, direction: "down", reason: "相手候補として評価上昇" },
    ],
    scratches: [],
    result: { status: "pending", message: "結果確定待ち。的中時はここが緑で強調されます。" },
  ...
  "tokyo-20260502-11": {
    ...
    nextPollSeconds: 60,
    updatedAt: "13:45",
    oddsMoves: [{ number: 2, name: "ダノンセンチュリー", previousOdds: 4.4, currentOdds: 3.8, direction: "down", reason: "前日オッズで支持上昇" }],
    scratches: [],
...
function buildBets(runners: PricedRunner[], bankroll: number, risk: number): Bet[] {
  ...
  if (mode === "balanced" || !third || runners.length < 5) return balancedBets;
...
export default function Home() {
  ...
  const filteredRaces = useMemo(() => (selectedVenue === "すべて" ? races : races.filter((race) => race.venue === selectedVenue)), [selectedVenue]);
  ...
  const setVenueTab = (venue: string) => {
    ...
    if (!nextRaces.some((race) => race.id === selectedRaceId)) setSelectedRaceId(nextRaces[0]?.id ?? races[0].id);
  };
    ...
    <main className="app-shell">
      <section className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <span className="status-pill"><CalendarDays size={16} />2026年5月2日・3日 / 全{races.length}R</span>
      ...
      <section className="page-grid">
        ...
        <section className="prediction-panel">
          ...
          {pricedRunners.length > 0 ? <>
            <div className="summary-grid">
              <article><Target size={19} /><span>本命</span><strong>{top.number} {top.name}</strong><small>勝率 {formatPercent(top.winProbability)} / 妥当 {top.fairOdds.toFixed(1)}倍</small></article>
              <article><Sparkles size={19} /><span>対抗</span><strong>{second.number} {second.name}</strong><small>勝率 {formatPercent(second.winProbability)}</small></article>
              <article><Gauge size={19} /><span>穴候補</span><strong>{third.number} {third.name}</strong><small>複勝圏 {formatPercent(third.placeProbability)}</small></article>
            ...
            <section className="ticket-panel"><div className="section-heading"><div><p>Tickets</p><h3>今買うならこの順</h3></div><ListChecks size={20} /></div><div className="ticket-list">{bets.map((bet) => <article key={`${bet.type}-${bet.selection}`}><div><span>{bet.type}</span><strong>{bet.selection}</strong><small>{bet.strategy ? `${bet.strategy} / ` : ""}{bet.note}{bet.tickets && bet.tickets > 1 && bet.unitStake ? ` / ${bet.tickets}点 x ${formatYen(bet.unitStake)}` : ""}</small></div><div className="ticket-side"><strong>{formatYen(bet.stake)}</strong><small>確率 {formatPercent(bet.probability)} / 想定 {bet.odds.toFixed(1)}倍</small></div></article>)}</div></section>
        ...
        <aside className="side-panel">
          <section className={`live-card result-${liveSnapshot.result.status}`}>
            ...
            <div className="result-banner">{liveSnapshot.result.status === "hit" ? <Trophy size={18} /> : <Activity size={18} />}<div><strong>{liveSnapshot.result.status === "hit" ? "的中" : liveSnapshot.result.status === "miss" ? "不的中" : liveSnapshot.result.status === "refund" ? "返還" : "結果待ち"}</strong><small>{liveSnapshot.result.message}{liveSnapshot.result.payout ? ` / 払戻 ${formatYen(liveSnapshot.result.payout)}` : ""}</small></div></div>
            <div className="live-alerts">{liveSnapshot.alerts.map((alert) => <span key={alert}>{alert}</span>)}</div>
            <div className="odds-move-list"><strong>オッズ変動</strong>{liveSnapshot.oddsMoves.length > 0 ? liveSnapshot.oddsMoves.map((move) => <article key={`${move.number}-${move.currentOdds}`}><TrendingDown size={16} /><div><b>{move.number} {move.name}</b><small>{move.previousOdds.toFixed(1)}倍 to {move.currentOdds.toFixed(1)}倍 / {move.reason}</small></div></article>) : <small>大きな変動なし</small>}</div>
            <div className="scratch-list"><strong>出走取消</strong>{liveSnapshot.scratches.length > 0 ? liveSnapshot.scratches.map((scratch) => <article key={`${scratch.number}-${scratch.announcedAt}`}><XCircle size={16} /><div><b>{scratch.number} {scratch.name}</b><small>{scratch.reason}</small></div></article>) : <small>取消なし</small>}</div>
          </section>
          ...
          <section className="source-card">
            ...
            <div className="metric-grid">
              <article><span>状態</span><strong>{apiBacktest?.status ?? backtestSummary.status}</strong></article>
              <article><span>期間</span><strong>{apiBacktest?.window ?? backtestSummary.window}</strong></article>
              <article><span>回収率</span><strong>{formatPercent(apiBacktest?.roi ?? backtestSummary.roi)}</strong></article>
              <article><span>的中率</span><strong>{formatPercent(apiBacktest?.hit_rate ?? backtestSummary.hitRate)}</strong></article>
              <article><span>賭け金</span><strong>{formatYen(apiBacktest?.total_stake ?? backtestSummary.totalStake)}</strong></article>
              <article><span>払戻</span><strong>{formatYen(apiBacktest?.total_payout ?? backtestSummary.totalPayout)}</strong></article>
              <article><span>最大DD</span><strong>{formatYen(apiBacktest?.max_drawdown ?? backtestSummary.maxDrawdown)}</strong></article>
              <article><span>買い目</span><strong>{(apiBacktest?.bets ?? backtestSummary.bets).toLocaleString("ja-JP")}</strong></article>
            </div>
            <p>{apiBacktest?.note ?? backtestSummary.note}</p>
          </section>

          <section className="source-card">
            <div className="section-heading"><div><p>Coverage</p><h3>特徴量カバレッジ</h3></div><Database size={20} /></div>
            <div className="coverage-list">
              {(apiCoverage.length > 0 ? apiCoverage : dataCoverage).map((item) => <article key={item.group}><div><strong>{item.group}</strong><small>{Array.isArray(item.fields) ? item.fields.join(" / ") : item.fields}</small></div><span>{"status" in item ? item.status : item.state}</span></article>)}
            </div>
          </section>
        </aside>
      </section>
